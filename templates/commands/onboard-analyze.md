...
# Onboard Analysis Module

Loaded by /buildflow-onboard after language detection and structural scan complete. Executes Steps 4–12 (deep analysis + write all knowledge files). Returns when all 6 knowledge files are written.
## Step 1: Map Modules & Risk — Module Boundaries, Load-Bearing Files & Risk Scores

Identify **bounded contexts** â€” groups of files that form a logical subsystem.
...
### Load-Bearing Module Identification
...
- **Fan-in** (how many other files import THIS file) â€” high = load-bearing
- **Fan-out** (how many files THIS file imports) â€” high = coupled

Files with fan-in â‰¥ 5 are **load-bearing** â€” mark as CRITICAL or HIGH.

---

### Risk Scoring

Score each file (1â€“5) on:
- Fan-in (0â€“1 = 1, 10+ = 5)
...
## Step 2: Patterns & Dependencies

Read 2â€“3 representative files per major module:
...
- Import order (stdlib â†’ 3rd party â†’ internal)
...
## Import Order
[stdlib â†’ 3rd party â†’ internal â€” example from actual codebase]

## Error Handling
[throw vs return â€” example from actual codebase]

## Async Pattern
[async/await / Promise chains â€” example]
...
## Test Layout
- [path pattern] â€” [unit/integration/e2e/fixture]

## Targeted Test Strategy
- Source file â†’ likely test file convention: [convention]
...
- [ENV_VAR] â€” [purpose, required/optional, evidence path]
...
## Symbol Caller Index
[SymbolName.method]    â†’ [file:line, file:line]
[SymbolName2]          â†’ [file:line]
```
...
Run ALL scans below. Do not skip any section â€” locale detection is required even if the project doesn't look like it has i18n, because locale evidence is often in config or static JSON files rather than in `src/`.
...
### 9c: Locale / i18n support â€” run every command regardless of language

This is the most commonly missed feature. Run all of these â€” the evidence may be in config, static JSON, docs, or dependency files rather than in application code.
...
grep -rn "English.*PortuguÃªs\|English.*æ—¥æœ¬èªž\|English.*í•œêµ­ì–´\|English.*ä¸­æ–‡\|English.*EspaÃ±ol\|language selector\|language switcher\|\[en\]\|\[fr\]\|\[ja\]\|\[ko\]\|\[zh\]" README*.md docs 2>/dev/null | head -30
...
1. **Locale catalog files found?** â†’ Any `.po`, `.mo`, `.resx`, `.arb`, `*.locale.json`, `messages*.properties`, `strings.xml`, `Localizable.strings`, or JSON files under `locales/`, `i18n/`, `translations/`, `lang/` â†’ **locale support exists**

2. **i18n library/framework found?** â†’ Any import of `react-i18next`, `next-intl`, `vue-i18n`, `flutter_localizations`, `I18n.t`, `ResourceBundle`, `django.utils.translation`, `gettext`, `go-i18n`, `Intl.message`, `NSLocalizedString`, etc. â†’ **locale support exists**

3. **Default locale** â†’ Look for: `defaultLocale`, `LANGUAGE_CODE`, `app.locale`, `default_locale`, `fallback_locale`, locale folder named `en/` or `en.json`, or the first entry in a `LANGUAGES = []` list

4. **Supported locales** â†’ Folder names under `locales/` or `lang/`, or file names like `fr.json`, `ja.json`, `.po` prefixes, or `supportedLocales: [...]` array

5. **Label catalogs** â†’ JSON files containing `label`, `placeholder`, `title`, `aria-label` as primary keys â†’ treat as locale evidence even if no i18n framework present

6. **If NO locale evidence found at all** â†’ mark `Status: NO` in PATTERNS.md (Locale Support section) with confidence `high` (not `unknown`) â€” the absence is a confirmed finding, not a gap in the scan
...
- Docs-only features â†’ mark `docs-only`; missing despite docs â†’ mark `documented_missing`
- Local support (Docker Compose, seeds, offline mode) is a first-class feature â€” always detect it
- **i18n/locale is a first-class feature â€” always scan for it using all 9c commands above, even for projects that appear monolingual**
...
### Locale Support
Status: YES / PARTIAL / NO  â† never leave as UNKNOWN; confirm YES or NO using scan results from Step 9c
Confidence: high / medium / low
Default locale: [en / fr / ja / UNKNOWN]
Supported locales: [en, fr, ja â€” derived from catalog filenames or supportedLocales array]
...
Catalog files:
- [path] â€” [locale code and purpose, e.g., "en.json â€” English translation catalog"]

Provider / config:
- [path:line] [i18n init, locale middleware, or language switcher]

Label / copy catalogs:
- [path] â€” [purpose, e.g., "UI button labels", "form placeholders"]

Localized docs:
- [path] â€” [locale and purpose, e.g., "README.ja-JP.md â€” Japanese README"]

Evidence:
- [path:line] [catalog / library import / config / provider]

Gaps:
- [missing fallback locale / no test coverage for locale switching / etc. â€” or NONE]
- [if Status is NO: "Confirmed â€” no locale catalogs, i18n libraries, or locale-specific file patterns found"]
...
Build: file â†’ imports list. Calculate fan-in and fan-out per file.
...
# Python â€” public symbols (no leading underscore)
grep -rn "^def [^_]\|^class [^_]\|^async def [^_]" src/ app/ --include="*.py" 2>/dev/null

# Go â€” exported (uppercase)
...
**Update `.buildflow/codebase/DEPENDENCIES.md`** using the Write tool to fill in the Import Graph and Symbol Caller Index sections (added in Step 8's template) with the data gathered in Steps 10aâ€“10c.
...
## Hotspots â€” Handle With Care
Files scored â‰¥ 3.5 risk. Review before any modification.
...
## Mapping Blind Spots
- [generated routes, dynamic imports, feature flags, missing tests â€” or NONE]
```

---

## Step 5: Write Knowledge Files — CODEBASE.md, PATTERNS.md, DEPENDENCIES.md, RISKS.md, TESTING.md, intel.json
...
- [npm/pnpm/yarn/pip/poetry/maven/gradle/go/cargo] â€” [evidence path]
...
### Boundary Violations
[files importing across module lines without public API â€” or NONE]
...
### Folder Structure
```
[annotated directory tree â€” one line per folder with its role]
```

### Generated / Static Assets
- [path] â€” [locale labels, fixtures, generated clients, public assets]
...
```json
{
  ...
  "locale_support": {
    ...
    "label_catalogs": ["[path to static label/copy JSON â€” or NONE]"],
    "localized_docs": ["[README.ja-JP.md / etc. â€” or NONE]"],
    "provider_files": ["[src/i18n/index.ts / etc.]"],
    "evidence": [
      { "file": "[path]", "line": 0, "note": "[catalog / library / config / provider]" }
    ],
    "gaps": ["[missing fallback locale / missing test coverage / etc. â€” or NONE]",
...
## Step 6: Verify & Complete — Secret Scan, Memory Update & Verification
...
Load-bearing files:  [N]  (fan-in â‰¥ 5)
High-risk hotspots:  [N]  (risk â‰¥ 3.5)
...
âš  Caution zones: [top 3 highest-risk files]
âœ“ Safe to modify:  [modules with low risk scores]
```
...
â†’ Next:  /buildflow-spec  (or /buildflow-modify for a targeted change)
