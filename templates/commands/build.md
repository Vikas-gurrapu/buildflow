...
- `.buildflow/epics/[epic]/PLAN.md` (index only)
- `.buildflow/epics/[epic]/waves/wave-[N].md` (current wave only — do not load other wave files)
- `.buildflow/epics/[epic]/CHECK.md`
- `.buildflow/epics/[epic]/STATE.md` (if exists - resume wave/status, risks, test strategy)
- `.buildflow/codebase/PATTERNS.md` (if exists)
- `.buildflow/codebase/CODEBASE.md` (if exists — only relevant sections for touched paths)
- `.buildflow/codebase/TESTING.md` (if exists — targeted test command and test layout)
- `.buildflow/codebase/RISKS.md` (if exists — only concerns relevant to touched paths)
...
Before exiting after each wave or full build, update `.buildflow/epics/[epic]/STATE.md` with:
...
Read `.buildflow/epics/[epic]/PLAN.md`.
...
```bash
grep -oE "AC-NF-[0-9]+" .buildflow/epics/[epic]/ACCEPTANCE.md | sort -u
grep -oE "AC-[0-9]+" .buildflow/epics/[epic]/ACCEPTANCE.md | grep -v "AC-NF" | sort -u
```

**Extract AC IDs referenced in PLAN.md tasks:**
```bash
grep -oE "AC-NF-[0-9]+" .buildflow/epics/[epic]/PLAN.md | sort -u
grep -oE "AC-[0-9]+" .buildflow/epics/[epic]/PLAN.md | grep -v "AC-NF" | sort -u
```
...
Were these intentionally excluded or accidentally dropped?
  ...
  [D] Deferred — mark as DEFERRED in CHECK.md, log to DEBT.md, proceed
...
| No framework, existing project with no tests | Warn: "⚠ No test framework found. Tests cannot be written until one is installed. Proceeding without tests — recommend adding [framework] before shipping." Log to `epics/[epic]/DEBT.md`: "No test framework — zero coverage." |
...
| None found | Warn once: "⚠ No build toolchain detected. Type safety and lint checks skipped." Log to `epics/[epic]/DEBT.md`. |
...
6. Update `.buildflow/epics/[epic]/CHECK.md` for the linked ACs:
...
If the user chooses defer or the AC remains blocked, update `CHECK.md` with `Status: DEFERRED` or `BLOCKED`, the failing command, and the next recommended fix location.
...
**Prerequisite:** Test Framework Profile from Step 2 must exist. If no framework was found and user chose to skip, mark this task's test output as SKIPPED and log to `epics/[epic]/DEBT.md`.
...
- **P (Proceed):** commit wave, log to `epics/[epic]/DEBT.md`: "Wave [N] coverage drop: [N]% → [M]% — [files] uncovered"
...
**After resolution:** log the deviation and chosen option in `epics/[epic]/PLAN.md` under a `## Deviations` section:
```markdown
## Deviations
...
1. Take a file snapshot — copy all files modified this wave into `.buildflow/snapshots/phase-[N]-wave-[N]-complete/`:
   ...
   └── phase-1-wave-2-complete/
       ...
       └── src/routes/auth.ts
   ```
2. Mark the wave complete in `epics/[epic]/PLAN.md` under the wave header:
   ```markdown
...
In both modes: mark wave complete in `epics/[epic]/PLAN.md` and proceed to next wave.

Update `.buildflow/epics/[epic]/CHECK.md` before moving to the next wave:
...
After the wave's file changes are known, classify structural changes against `.buildflow/codebase/CODEBASE.md` if it exists:

| Category | Trigger |
|----------|---------|
| `new_dir` | new directory not represented in `CODEBASE.md` |
...
After final integration check, update `.buildflow/epics/[epic]/CHECK.md`:
