...
| Command | Agent | Purpose | Token Cost |
|---------|-------|---------|-----------|
| `/buildflow-onboard` | Cartographer | 5-lens analysis (arch/quality/security/data/features), **symbol-level import graph**, feature inventory, local support map, locale/i18n map, risk scores, queryable `intel.json` | ~45K |
...
| **Features** | User-facing capabilities, local/offline/dev support, locale/i18n support across JS, Java, Go, Python, Ruby, PHP, .NET, mobile, static JSON/catalog files, scripts, routes, screens, tests, docs |
...
Onboarding also writes `.buildflow/codebase/FEATURES.md`, `intel.json.features[]`, `local_support`, and `locale_support`, so planning can preserve existing capabilities such as local dev workflows and locale/i18n support instead of accidentally missing or rebuilding them. Locale mapping checks static catalogs plus language-specific APIs/dependencies like Java `ResourceBundle`, Spring `MessageSource`, Go `golang.org/x/text`, Python `gettext`/Babel, Rails `I18n.t`, Laravel `trans()`, .NET `.resx`, Flutter `.arb`, iOS `Localizable.strings`, and Android `strings.xml`.
